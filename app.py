...
MARGIN_PERCENT = 0.05       # 5% от баланса
LEVERAGE = 3                # Плечо x3
ACCOUNT_COIN = "USDT"
...
def set_symbol_leverage(symbol):
    try:
        ...
        print(f"--> Плечо по {symbol} уже установлено или ошибка: {e}")
...
def execute_signal(symbol, target_side):
    """
    Обработка входа (Long) или переворота (Long -> Short)
    """
    with trade_lock:
        set_symbol_leverage(symbol)
        ...
        if position:
            ...
            # Закрываем встречную позицию перед открытием новой (переворот)
            closing_side = "Sell" if current_side == "Buy" else "Buy"
            print(f"--> Переворот по {symbol}: закрываем {current_side} ({current_size})")
            ...
            print(f"--> Ответ закрытия при перевороте: {close_resp}")
...
def close_position(symbol):
    """
    Строгое закрытие позиции (соответствует strategy.close для Short при зеленой свече)
    """
    with trade_lock:
        position = get_position(symbol)
        if not position:
        ...
        print(f"--> Позиция {symbol} закрыта по сигналу: {response}")
...
def webhook():
    try:
        ...
        # Вход в Long (или открытие по стратегии)
        if action in ["buy", "купить", "long", "entry"]:
            result = execute_signal(symbol, "Buy")
            return jsonify(result), 200

        # Переворот в Short (появление красной свечи при лонге)
        if action in ["sell", "продать", "short"]:
            result = execute_signal(symbol, "Sell")
            return jsonify(result), 200

        # Закрытие позиции (появление зеленой свечи при шорте -> strategy.close)
